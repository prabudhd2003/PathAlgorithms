...
WIDTH = 800
MARGIN = 100  # Margin for text at the bottom
WIN = pygame.display.set_mode((WIDTH, WIDTH + MARGIN))
pygame.display.set_caption("Path Finding Algorithm (A* and Dijkstra)")
...
def draw_grid(win, rows, width):
    ...
    for i in range(rows):
        ...
        for j in range(rows):
            pygame.draw.line(win, GREY, (j * gap, 0), (j * gap, width - MARGIN))
...
def main(win, width):
    ...
    while run:
        ...
        if algorithm_type is None:
            display_text(win, "Click on Grid to make 'Start' and 'Stop' points. Click again to make barrier", 10, WIDTH + 20)
